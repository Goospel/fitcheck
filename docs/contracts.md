...
⚠️ **보내는 모양과 받는 모양이 다르다.** 치수 4개는 요청에서 **평평하고**, 응답에서만
`measurements` 로 감싸진다.

```json
// PUT 이 받는 것 — 치수가 최상위에 평평하게 온다
{ "height": 175, "weight": 70, "gender": "남성",
  "shoulder": 45.0, "chest": 96.0, "preferredGrade": "레귤러핏" }
```

⚠️ **`GET` 응답을 그대로 `PUT` 하면 실측 넷이 조용히 사라진다.** `measurements` 는 요청에
없는 필드라 버려지고 치수가 다시 「추정」으로 돌아가는데, **에러가 안 난다** — 정확도만
떨어져 있다. 되돌려 보낼 땐 평평하게 풀고, `source` 가 `실측` 인 것만 싣는다(추정값을
실어 보내면 그때부터 실측으로 집계된다).
